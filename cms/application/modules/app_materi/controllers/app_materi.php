...
class App_materi extends MX_Controller {
    
    # property
    public $root;
    public $app_name            = ''; 
    public $pageTitle           = ''; 
    public $initialPage         = 'Materi';
    # initial file image
    public $imgDir              = 'uploads/materi';
    public $initial_id;
    ...
    public function index(){
        ...
        $params['datadb']      =  $this->coredb->binDataMateri();   
        $params['x']           =  $this->coredb->binDataX();   
        $params['xi']          =  $this->coredb->binDataXi();   
        $params['xii']         =  $this->coredb->binDataXii();   
        ...
        if( $_POST ){
             if( $this->form_validation->run(REG_VALIDATION) !== FALSE ){
              // var_dump($_FILES['silabus_kls_x']);die;
               ...
               if( $_FILES['file_vii']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['file_vii']) !== FALSE ){
                        $_POST['file_vii']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['file_ix']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['file_ix']) !== FALSE ){
                        $_POST['file_ix']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}
             # -------------------------------------------------------------------------------------               
                ...
                if( $this->uidcontroll->insertData('satria_smk_materi', bindProcessing($_POST) ) !== FALSE){
        ...
        if( $_POST ){
             if( $this->form_validation->run(REG_VALIDATION) !== FALSE ){
              // var_dump($_FILES['silabus_kls_x']);die;
               ...
               if( $_FILES['file_ak']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['file_ak']) !== FALSE ){
                        $_POST['file_ak']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['file_ap']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['file_ap']) !== FALSE ){
                        $_POST['file_ap']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['file_mm']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['file_mm']) !== FALSE ){
                        $_POST['file_mm']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['file_pm']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['file_pm']) !== FALSE ){
                        $_POST['file_pm']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}
             # -------------------------------------------------------------------------------------               
