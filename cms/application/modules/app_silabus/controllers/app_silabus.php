        ...
        if( $_POST ){
             if( $this->form_validation->run(REG_VALIDATION) !== FALSE ){
              // var_dump($_FILES['silabus_kls_x']);die;
               ...
               if( $_FILES['silabus_kls_1']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['silabus_kls_1']) !== FALSE ){
                        $_POST['silabus_kls_1']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['silabus_kls_2']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['silabus_kls_2']) !== FALSE ){
                        $_POST['silabus_kls_2']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['silabus_kls_3']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['silabus_kls_3']) !== FALSE ){
                        $_POST['silabus_kls_3']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['silabus_kls_4']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['silabus_kls_4']) !== FALSE ){
                        $_POST['silabus_kls_4']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['silabus_kls_5']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['silabus_kls_5']) !== FALSE ){
                        $_POST['silabus_kls_5']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}

               if( $_FILES['silabus_kls_6']['name'] !== "")
               {
                   $this->load->library('fileupload');
                   $this->fileupload->path_directory = $this->imgDir;
                   if( $this->fileupload->init($_FILES['silabus_kls_6']) !== FALSE ){
                        $_POST['silabus_kls_6']     = $this->fileupload->path_directory;
                   }
               }else{$this->messagecontroll->delivered('msg_warning', validation_errors());}
             # -------------------------------------------------------------------------------------               
                ...
                if( $this->uidcontroll->insertData('satria_mi_silabus', bindProcessing($_POST) ) !== FALSE){
